...
- **`--use-llm`** routes through `AgenticOrchestrator` (`llm_orchestrator.py`), **not**
  the older single-shot `LLMOrchestrator` (that class still exists but is only
  reachable from tests that monkeypatch `_call_llm` directly — `planner.py`'s
  `plan_timeline` never calls it). `AgenticOrchestrator` is a 4-phase
  workflow-of-agents run: Phase 0 structural (deterministic tighten + beat
  segmentation), Phase 1 optional clarification (`--interactive` only — Clarifier
  asks 2-4 questions via the terminal), Phase 2 content analysis (Analyst reads the
  transcript excerpt + beat map), Phase 3 a bounded reflection loop (Planner
  proposes a `SkillPlan` → cheap trial `compose_timeline` with no FFmpeg → Critic
  reviews for coherence + taste → Planner revises; capped at 2 rounds, last plan
  wins regardless), Phase 4 final compose. That means a plain `--use-llm` run (no
  `--interactive`) can issue up to ~4 OpenAI calls (Analyst, Planner, Critic,
  one revision) instead of the legacy path's 1 — budget for that cost/latency
  difference. It still **falls back to the deterministic plan on any failure**
  in any phase (no/invalid API key, bad JSON, unknown skill), writing
  `planner_error.txt`. So a `--use-llm` run with no key still produces a video —
  check `edit_plan.json`'s `reason` (free text = a real LLM call happened; the
  deterministic template = it fell back) and whether `planner_error.txt` exists.
  `--interactive` (`-i`) implies `--use-llm` and additionally runs Phase 1.
...
> `DeterministicOrchestrator` / `AgenticOrchestrator` (`orchestrator.py` /
...
- `planner.py` — `plan_edits`/`plan_timeline` entry points. Both paths build a coherence-validated `Timeline` and adapt to an `EditPlan`: deterministic → `DeterministicOrchestrator`; `use_llm` → `AgenticOrchestrator` (`interactive=True` additionally runs its Phase 1 clarification). `DeterministicPlanner` remains as a thin `EditPlan`-returning shim (used directly by tests; it does not tighten). `build_captions` is re-exported here from `skills.caption` for backward compatibility.
...
- `llm_orchestrator.py` — two orchestrators sharing the `plan_timeline()` interface. **`AgenticOrchestrator`** (current — `planner.py`'s `--use-llm` path) is a workflow-of-agents: Phase 0 structural skills (tighten + `BeatSegmenterSkill`, deterministic, always run first) → Phase 1 optional clarification (`--interactive` only, via the Clarifier + `intent.CliIO`) → Phase 2 content analysis (Analyst) → Phase 3 a reflection loop capped at 2 rounds (`_plan_with_reflection`: Planner proposes a `SkillPlan`, `_trial_compose` runs `compose_timeline` with no FFmpeg to get coherence warnings, Critic reviews taste + violations and either approves or hands back fixes for the Planner to revise) → Phase 4 final compose (structural + the chosen decoration skills) via the shared `compose_timeline`. **`LLMOrchestrator`** (legacy) is the one-shot predecessor — one LLM call → `SkillPlan` → `compose_timeline` — kept only because existing tests monkeypatch its `_call_llm`; nothing in `cli.py`/`planner.py` reaches it anymore. Both **fall back to `DeterministicOrchestrator` on any failure** in any phase (raw response/exception → `planner_error.txt`). Because the LLM only ever chooses skills/params from the toolbox, it cannot corrupt timings or export dims (a safety win over the old EditPlan-JSON approach) — true for every phase of the agentic workflow too, since the Planner/Critic/Analyst/Clarifier all emit typed Pydantic output (`toolbox.SkillPlan`, `intent.ContentAnalysis`, `intent.PlanCritique`, `intent.QuestionSet`), never raw EditPlan JSON.
- `agent.py` — the agent primitives shared by every worker. A single `Agent` class (composition over inheritance: Clarifier/Analyst/Planner/Critic are the same class configured with different `instructions`/`output_schema`/`tools`, not subclasses). `Context` is injected at `invoke()` time rather than baked into the prompt, so one `Agent` instance is reusable across calls and tests. `LLMClient` is the one seam to a real provider (`OpenAIClient`, gated by `concurrency.openai_semaphore`) vs. tests (`FakeClient`), matching the project's offline-first ethos. `Agent.invoke()` does one bounded self-repair attempt on JSON parse failure — never an infinite retry loop, and a still-unparseable result comes back as `AgentResult.parsed = None` rather than silently swallowed.
- `workers.py` — factory functions (`make_clarifier`/`make_analyst`/`make_planner`/`make_critic`) that bind the static system prompts to a `LLMClient` and produce a configured `Agent`. All orchestration/control-flow lives in `llm_orchestrator.AgenticOrchestrator`, not here — this module is prompts + wiring only. `_PLANNER_INSTRUCTIONS` is what tells the Planner to write real hook copy that teases rather than restates the transcript, and to leave `CaptionParams.emphasize` empty far more often than not.
- `intent.py` — the agentic layer's typed I/O: `QuestionSet`/`Question` (Clarifier output), `CreativeIntent` (renders the user's CLI answers back into prompt text), `ContentAnalysis` (Analyst output), `PlanCritique` (Critic output: `approved` + `taste_issues`/`fixes`), and `CliIO` (the terminal Q&A used by `--interactive`'s Phase 1). Also holds the unrelated-but-colocated `SetupInterview`/`SetupChoices` (the full `--interactive` setup wizard for style/layout/tighten/captions) and `STYLE_DESCRIPTIONS` (used by `cli.py`'s `_prompt_style_choice`).
